...
public class AppuserController extends BaseController {
	...
	public ModelAndView goAddS(){
		...
		pd = this.getPageData();
		try {
			mv.setViewName("system/swordsman/swordsman_edit");
			mv.addObject("pd", pd);
		} catch (Exception e) {
	...
	@RequestMapping(value = "/listJbx")
	public ModelAndView listJbx(Page page) {
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		try {
			pd = this.getPageData();
			...
			page.setPd(pd);
			List<PageData> userList = swordsManService.listPdPageSwordsMan(page); // 列出用户列表

			mv.setViewName("system/swordsman/swordsman_list");
			mv.addObject("userList", userList);
			mv.addObject("pd", pd);
			mv.addObject(Const.SESSION_QX, this.getHC()); // 按钮权限
		} catch (Exception e) {
		...
		try {
			if (Jurisdiction.buttonJurisdiction(menuUrl, "cha")) {
				...
				for (int i = 0; i < userList.size(); i++) {
					...
					vpd.put("var5", userList.get(i).getString("ADDRESS")); // 5
					vpd.put("var6", userList.get(i).getString("BNAME")); // 6
					vpd.put("var7", userList.get(i).getString("TYPE")); // 7
					vpd.put("var8", userList.get(i).getString("LAST_LOGIN")); // 7
					varList.add(vpd);
				}
