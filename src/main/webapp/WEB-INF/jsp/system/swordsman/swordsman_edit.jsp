...
<body>
	<form action="happuser/${msg }.do" name="userForm" id="userForm" method="post">
		...
		<table>		
			<tr>
				<td><input type="text" name="USERNAME" id="name" value="${pd.USERNAME }" placeholder="这里输入大侠名" title="大侠名"/></td>
			</tr>
			<tr>
				<td>
					<select name="STATUS" title="大侠性别">
					<option value="1" <c:if test="${pd.STATUS == '1' }">selected</c:if> >男大侠</option>
					<option value="0" <c:if test="${pd.STATUS == '0' }">selected</c:if> >女大侠</option>
					</select>
				</td>
			</tr>
